...
class TestProcessingRfbV1(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.file_update = open(
            f"{settings.BASE_DIR}/cnpj/tests/files/Test CNPJ Update.xlsx", "rb"
        )
        self.file_new = open(
            f"{settings.BASE_DIR}/cnpj/tests/files/Test CNPJ New.xlsx", "rb"
        )
        self.file_less_column = open(
            f"{settings.BASE_DIR}/cnpj/tests/files/Test CNPJ less column.xlsx", "rb"
        )
    ...
    def test_new_cnpj(self):
        ...
        processing_rfb.save()
        df = pd.read_excel(
            f"{settings.BASE_DIR}/cnpj/tests/files/Test CNPJ New.xlsx", decimal=","
        )
        ...
        self.assertEqual(
            CnpjCei.objects.filter(identification_number__in=cnpj_to_search).count(), 2
        )
        ...
        self.assertEquals(
            cnpj_new.date_registration_status,
            datetime.strptime("2020-06-13 00:00:00", "%Y-%m-%d %H:%M:%S").date(),
        )
        self.assertEquals(cnpj_new.reason_situation, 0)
        self.assertEquals(
            cnpj_new.start_activities,
            datetime.strptime("2020-06-13 00:00:00", "%Y-%m-%d %H:%M:%S").date(),
        )
